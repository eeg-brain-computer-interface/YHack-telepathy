function eeg_data = dataimport_forruth(basename)
...
rawdata = load([filepath pfiles(1).name]);
data.ans = timeseries(rawdata.eeg);
...
% % pass filter
 EEG = pop_eegfilt(EEG, 0, 1, [], [0], 0, 0, 'fir1',0); % does a 30 Hz
% % low pass filter
 EEG = pop_iirfilt( EEG, 40, 48, 0, 1); % does a 60 Hz notch filter
